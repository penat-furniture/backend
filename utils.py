import os
from typing import Optional, Union
import pandas as pd
import numpy as np
import chromadb
from oracul import PenatOracul

SERVER_URL = os.getenv("SERVER_URL", "https://cdn.penat.su")
PATH_TO_LINKS = '/Users/ksc/penat/new_data/links.csv'
PATH_TO_CHROMA_CACHE = '/Users/ksc/penat/room_update_chroma_cache'
AVAILABLE_CATEGORIES = ['bed', 'chair', 'couch', 'table', 'plant', 'closet'] # chroma already has closet

def resolve_path(name: str, image_type: str = "chair") -> str:
    return f"{SERVER_URL}/all/{name}" # This needs CDN to have the correct names

chroma_client = chromadb.PersistentClient(path=PATH_TO_CHROMA_CACHE)
global_oracul = PenatOracul(
    collection=chroma_client.get_or_create_collection("common"),
    price_csv_path=PATH_TO_LINKS,
    not_found_final_res={
        "name": "Не найдено",
        "image": resolve_path("not_found.png", "icons"),
        "link": None,
        "price": None,
        "limits": {"min": 0, "max": 1e6, "loaded": True},
    },
    main_type='chair',
    panel_size=4
)

def get_limits() -> dict:
    res = {}
    for el in AVAILABLE_CATEGORIES:
        res[el] =  {
            "min": 0,
            "max": 1e10
        }
    return res

def get_images(room, step: str = "1", image: Optional[str] = None, limits: Optional[dict] = None,
               ) -> Union[list, dict]:
    print("STEP:", step)
    if step == "1" or int(step) in global_oracul.distance_func.keys():
        oracul_outp = global_oracul.run_general_step(step_n=int(step), image=image, room=room)
        return [
            {
                "name": name,
                "image": resolve_path(name + '.jpeg', "chair"),
            }
            for name in oracul_outp
        ]
    else:
        print(limits)
        oracul_outp = global_oracul.run_final_step(image=image, limits=limits, room=room)
        res = {}
        
        for category_name in oracul_outp: 
            if oracul_outp[category_name]['price'] is None: # not found obj
                res[category_name] = oracul_outp[category_name]
            else:
                res[category_name] = {
                    "name": oracul_outp[category_name]['name'],
                    "image": resolve_path(oracul_outp[category_name]['name'] + '.jpeg', category_name), 
                    "link": oracul_outp[category_name]['link'],
                    "price": oracul_outp[category_name]['price'],
                    "limits": limits[category_name]
                }
        return res
